...
`;
  ...
  function pushState() {
    ...
    let lastMat =
      stateStack.length > 1
        ? stateStack[stateStack.length - 2].id + "currentMaterial"
        : "material";
    appendSources("vec3 " + getCurrentPos() + " = " + lastP + ";\n");
    appendColorSource(
      "Material " + getMainMaterial() + " = " + lastMat + ";\n"
    );
    appendColorSource(
      "Material " + getCurrentMaterial() + " = " + lastMat + ";\n"
    );
  ...
  function mult(a, b) {
    ...
    return new makeVarWithDims(
      "(" + collapseToString(a) + "*" + collapseToString(b) + ")",
      dims
    );
  }
  ...
  function reset() {
    if (stateStack.length > 1) {
      appendSources(
        getCurrentPos() + " = " + stateStack[stateStack.length - 2].id + "p;\n"
      );
    } else {
  ...
  function metal(val) {
    ...
    appendColorSource(
      getCurrentMaterial() + ".metallic = " + collapseToString(val) + ";\n"
    );
  }
  ...
  function shine(val) {
    ...
    appendColorSource(
      getCurrentMaterial() + ".roughness = 1.0-" + collapseToString(val) + ";\n"
    );
  }
  ...
  function occlusion(amount) {
    ...
    appendColorSource(
      getCurrentMaterial() +
        ".ao = mix(1.0, occlusion(op,normal), " +
        amt +
        ");\n"
    );
  }
  ...
  function input2D(
    ...
    let xyExist = [value, min, max].reduce(
      (acc, curr) => acc && "x" in curr && "y" in curr
    );
    if (!xyExist) {
      compileError("input2D: value, min, and max must be a vec2");
    }
  ...
  function get2DCoords() {
    return makeVarWithDims(
      "vec2((gl_FragCoord.x/resolution.x-0.5)*(resolution.x/resolution.y),gl_FragCoord.y/resolution.y-0.5)",
      2,
      false
    );
  }
  ...
  function grid(num = 2, scale = 0.2, roundness = 0.05) {
